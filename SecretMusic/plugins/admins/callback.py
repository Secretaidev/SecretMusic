...
from SecretMusic import YouTube, app
...
from SecretMusic.utils.inline import close_markup, stream_markup, stream_markup_timer
from SecretMusic.utils.inline.help import help_pannel_page1, help_pannel_page2, help_pannel_page3, help_pannel_page4
from SecretMusic.utils.stream.autoclear import auto_clean
...
async def fork_repo_callback(client, query):
    await query.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("🚀 Fᴏʀᴋ Rᴇᴘᴏ", url="https://github.com/Secretaidev/SecretMusic/fork"),
                    InlineKeyboardButton("⚡ Hᴇʀᴏᴋᴜ Dᴇᴘʟᴏʏ", url="https://dashboard.heroku.com/new?template=https://github.com/Secretaidev/SecretMusic")
                ],
                [
...
from SecretMusic.utils.inline.start import about_panel
from strings import get_string
...
from pyrogram import Client, filters
from pyrogram.types import InlineKeyboardMarkup
from SecretMusic.utils.inline.start import owner_panel  # Import owner_panel function
from strings import get_string
from config import BANNED_USERS
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await SecretCall.pause_stream(chat_id)
        ...
        await SecretCall.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await SecretCall.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await SecretCall.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                await SecretCall.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            ...
            try:
                await SecretCall.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            ...
            try:
                await SecretCall.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
